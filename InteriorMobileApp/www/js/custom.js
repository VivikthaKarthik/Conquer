(function ($) {
  ...
  $(document).ready(function () {
    ...
    window.LoadDataFromAPI = function () {
      alert("JS Calls Working");
    };
    ...
    window.LoadMainMenu = function (pageName) {
      get(
        "Common",
        "GetMainMenu?pageName=" + pageName,
        function (response) {
          if (response != null && response.isSuccess) {
            var menuItems = response.result;

            if (menuItems != null && menuItems.length > 0) {
              const container = document.getElementById("main-menu");
              container.innerHTML = menuItems; // Clear any existing content
            }
          }
        },
        function (error) {
          alert("There is an issue in calling WebAPi. Please try again");
